...
{
    [Serializable]
    public class LinearAutoencoder
    {
        ...
        public LinearAutoencoder(int hidden, int visible, double[][] data, bool tiedWeight = false, double lambda = 3e-3,
          double sparsity_param = .035, double beta = 5, int batchNumber = 300,
          double learningRate = 0.09, int iteration = 5000, double momentum = 0.5)
        {
            ...
            double lambda, double beta, double[] rho)
        {
            ...
            var delta3 = X.Subtract(h).Multiply(-1);
